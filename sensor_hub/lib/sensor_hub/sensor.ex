defmodule SensorHub.Sensor do
  ...
  def fields(VEML6075), do: [:uva_raw, :uvb_raw, :uvia, :uvib, :uvi]

  def read_fn(SGP30), do: fn -> SGP30.state() end
  def read_fn(BMP280), do: fn -> BMP280.measure(BMP280) end
  def read_fn(VEML6075), do: fn -> VEML6075.get_measurement() end
  ...
  def convert_fn(VEML6075) do
    fn reading ->
      Map.take(reading, [:uva_raw, :uvb_raw, :uvia, :uvib, :uvi])
    end
  end
